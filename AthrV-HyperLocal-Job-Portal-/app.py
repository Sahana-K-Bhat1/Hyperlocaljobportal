    ...
    if request.method == 'POST':
        ...
        if check_seeker_email(email):
            if len(password1) and password1 == password2:
                get1 = save_seeker_details(username, email, password1)
                session['details'] = get1
    ...
    if request.method == 'POST':
        ...
        if check_creator_email(post_email):
            if len(post_password) and post_password == post_confirm_pass:
                get1 = save_creator_details(post_name, post_email, post_password)
                session['c_details'] = get1
...
def seeker_dashboard():
    s = rating_list()
    print(s)
    if (str(s) == "0") or (str(s) == "None"):
        return render_template('seeker_dashboard.html')
    else:
        return render_template('seeker_dashboard.html', rating=s)


@app.route('/creator_dashboard')
@login_required
def creator_dashboard():
    k = fetchcreatorrating()
    print(k)
    if (str(k) == "0") or (str(k) == "None"):
        return render_template('creator_dashboard.html')
    else:
        return render_template('creator_dashboard.html', value10=k)
...
def seeker_profile_edit():
    ...
    if request.method == 'POST':
        ...
        if username and address:
            ...
            return redirect(url_for('seeker_dashboard'))
    return render_template('seeker_profile_edit.html', address=s_address, mobile=s_mobile, education=s_education)
...
def remove_seeker_skill():
    if request.method == "POST":
        ...
        if request.form["r_skill"] == "Add_skill":
            ...
            print(name)#.getlist("Remove_skill", type=None) # to get all the values that are selected in remove skill option
...
def rating_list():
    email = current_user.email
    id = fetchseekerratingid(email)
    list1 = []
    seeker_rating = fetchseekerrating1(id)
    if seeker_rating:
        for y in seeker_rating:
            if 'None' in str(y[0]):
                continue
            else:
                list1.append(*y)
...
def fetchcreatorrating():
    email = current_user.email
    id = fetchcreatorratingid(email)
    data = fetchcreatorrating1(id)
    list1 = []
    if data:
        for x in data:
            data1 = fetchcreatorrating2(data, x)
            if data1:
                for y in data1:
                    if 'None' in str(y[0]):
                        continue
...
@app.route("/close1")
def close1():
    return redirect(url_for('seeker_profile_edit'))
...
def get_j():
    if request.method=='POST':
        global job
        job = request.form['search_job']
        print(job)
        global job_n
        job_n='%'+job+'%'
        print(job_n)
        email = current_user.email
        seeker_id = get_id(email)
        print(seeker_id)
        job_list=search_jobs(job_n,seeker_id) #to search the job which seeker not applied
        print(job_list)
        if (job_list):
            comp=[]
        else:
            comp=['one']
        # yes is used when there is no result for the search, if we don't use this, then at the begining (before the search) it displays errror message to avoid that, we are using this
        return render_template('search_job.html', job_list=job_list,input_value=job,yes=comp)  # input_value shows the seeker what he searched(while clicking submit, inputs disappears to show the searched value, this is used.)


@app.route('/apply_search',methods=['GET','POST'])
def add_s_j():
    if request.method=='POST':
        app_job_id = request.form['Apply_search_job']
        print(app_job_id)
        email=current_user.email
        seeker_id=get_id(email)
        add_search_jobs(app_job_id,seeker_id)
        job_list = search_jobs(job_n, seeker_id)
        return render_template('search_job.html', job_list=job_list,input_value=job)

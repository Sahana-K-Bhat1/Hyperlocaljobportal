...
def fetchseekerratingid(email):
    myconn=psycopg2.connect("postgres://bijylgibfwwhqv:953577e1fae84feb07ad8df06b81d42b0b8002a6ab9a6838e10eaf19075614c7@ec2-35-174-35-242.compute-1.amazonaws.com:5432/d3gqb6tqvfmfdv")
    cur=myconn.cursor()
    cur.execute("select seeker_id from seeker_details where email='%s'" % email)
    c = cur.fetchall()
    d = c[0]
    id = d[0]
    return id

def fetchseekerrating1(id):
    myconn=psycopg2.connect("postgres://bijylgibfwwhqv:953577e1fae84feb07ad8df06b81d42b0b8002a6ab9a6838e10eaf19075614c7@ec2-35-174-35-242.compute-1.amazonaws.com:5432/d3gqb6tqvfmfdv")
    cur=myconn.cursor()
    cur.execute("select seeker_rating from ratings where seeker_id='%d'" % id)
    seeker_rating = cur.fetchall()
    return seeker_rating

def fetchcreatorratingid(email):
    myconn=psycopg2.connect("postgres://bijylgibfwwhqv:953577e1fae84feb07ad8df06b81d42b0b8002a6ab9a6838e10eaf19075614c7@ec2-35-174-35-242.compute-1.amazonaws.com:5432/d3gqb6tqvfmfdv")
    cur=myconn.cursor()
    cur.execute("select creator_id from creator_details where email='%s'" % email)
    c = cur.fetchall()
    d = c[0]
    id = d[0]
    return id

def fetchcreatorrating1(id):
    myconn=psycopg2.connect("postgres://bijylgibfwwhqv:953577e1fae84feb07ad8df06b81d42b0b8002a6ab9a6838e10eaf19075614c7@ec2-35-174-35-242.compute-1.amazonaws.com:5432/d3gqb6tqvfmfdv")
    cur=myconn.cursor()
    cur.execute("select job_id from posted_jobs where creator_id='%d'" % id)
    data = cur.fetchall()
    print(data)
    return data

def fetchcreatorrating2(data,x):
    myconn=psycopg2.connect("postgres://bijylgibfwwhqv:953577e1fae84feb07ad8df06b81d42b0b8002a6ab9a6838e10eaf19075614c7@ec2-35-174-35-242.compute-1.amazonaws.com:5432/d3gqb6tqvfmfdv")
    cur=myconn.cursor()
    cur.execute("select creator_rating from ratings where job_id=%d" % (x))
    data1 = cur.fetchall()
    return data1
...
def search_jobs(j_name,seek_id):
    con = psycopg2.connect(
        'postgres://bijylgibfwwhqv:953577e1fae84feb07ad8df06b81d42b0b8002a6ab9a6838e10eaf19075614c7@ec2-35-174-35-242.compute-1.amazonaws.com:5432/d3gqb6tqvfmfdv')
    curr = con.cursor()
    print("j_name is",j_name)
    curr.execute("select  * from posted_jobs where job_name like '%s' and job_id not in(select job_id from applied_jobs where seeker_id='%d')"%(j_name,seek_id))
    jobs=curr.fetchall()
    curr.execute("select job_id, status from applied_jobs where seeker_id='%d'"%(seek_id))
    get2=curr.fetchall()
    return jobs


def add_search_jobs(j_id,seek_id):
    con = psycopg2.connect(
        'postgres://bijylgibfwwhqv:953577e1fae84feb07ad8df06b81d42b0b8002a6ab9a6838e10eaf19075614c7@ec2-35-174-35-242.compute-1.amazonaws.com:5432/d3gqb6tqvfmfdv')
    curr = con.cursor()
    print(j_id,seek_id)
    curr.execute("insert into applied_jobs (job_id,seeker_id,status) values('%d','%d','%s')"%(int(j_id),int(seek_id),'A'))
    con.commit()
...
def get_id(s_email):
    con = psycopg2.connect(
        'postgres://bijylgibfwwhqv:953577e1fae84feb07ad8df06b81d42b0b8002a6ab9a6838e10eaf19075614c7@ec2-35-174-35-242.compute-1.amazonaws.com:5432/d3gqb6tqvfmfdv')
    curr = con.cursor()
    add = "Select  seeker_id from Seeker_details where email='%s'" % s_email  # to get the seeker_id using email
    curr.execute(add)
    records = curr.fetchall()
    r_id = records[0]
    return int(r_id[0])
